...
# --- VERIFIED PRICE UPDATES / ADDITIONS (checked 2026-03-02) ---

updates_new <- tribble(
    ~Product, ~Brand, ~Price_ZAR, ~Serving_Size, ~Carbs_per_Serving, ~Source_Link,

    # Maurten (Cyclelab confirmed for gels; Maurten SA confirmed for drink mixes)
    "Maurten Gel 100", "Maurten", 74.99, "40 g gel", 25, "https://www.cyclelab.com/product/4005624-gel-maurten-100-each",
    "Maurten Gel 100 CAF 100", "Maurten", 84.55, "40 g gel", 25, "https://www.cyclelab.com/product/4005626-gel-maurten-100-caf-each",
    "Maurten Drink Mix 160", "Maurten", 50.00, "40 g sachet", 40, "https://maurten.co.za/catalogue/maurten-drinkmix-160/",
    "Maurten Drink Mix 320", "Maurten", 75.00, "80 g sachet", 80, "https://maurten.co.za/catalogue/maurten-drinkmix-320/",
    "Maurten Drink Mix 320 CAF 100", "Maurten", 82.00, "80 g sachet", 80, "https://maurten.co.za/catalogue/sachet-maurten-drink-mix-320-caf/",

    # SiS (Science in Sport SA)
    "SiS GO Isotonic Energy Gel", "SiS", 44.00, "60 ml gel", 22, "https://scienceinsport.co.za/go-isotonic-energy-gel-60ml-single-unit-fruit-salad",
    "SiS Beta Fuel Dual-Source Energy Gel", "SiS", 74.00, "60 ml gel", 40, "https://scienceinsport.co.za/beta-fuel",
    "SiS Beta Fuel + Nootropics Gel", "SiS", 80.00, "60 ml gel", 40, "https://scienceinsport.co.za/beta-fuel",

    # 226ERS (226ERS SA)
    "226ERS High Fructose Gel", "226ERS", 92.99, "80 g gel", 55, "https://226ers.co.za/products/high-fructose-gel-energy-gel-80g",

    # STYRKR (Vivo Vita SA)
    "STYRKR GEL50", "STYRKR", 85.00, "60 g gel", 50, "https://vivovitasport.com/product-category/styrkr/energy-gels/",

    # AMACx (Gear-Change SA)  ---- corrections + new items
    "AMACx Turbo Gel", "AMACx", 79.00, "60 ml gel", 40, "https://gearchange.co.za/products/nutrition-amacx-turbo-gel",
    "AMACx Drink Gel", "AMACx", 66.00, "60 ml gel", NA, "https://gearchange.co.za/products/nutrition-amacx-drink-gel",
    "AMACx Turbo Ice Gel", "AMACx", 90.00, "60 ml gel", NA, "https://gearchange.co.za/products/nutrition-amacx-turbo-ice-gel",

    # NEVERSECOND (new-to-your-list; SA retailer example)
    "NEVERSECOND C30 Energy Gel", "NEVERSECOND", 90.00, "gel (single)", 30, "https://complete-cyclist.com/products/neversecond-c30-energy-gel"
)

# Precision Fuel – Carbon Endurance price updated (single unit)
precision_updates <- tribble(
    ~Product, ~Brand, ~Price_ZAR, ~Serving_Size, ~Carbs_per_Serving, ~Source_Link,
    "Precision Fuel 30g Gel", "Precision Fuel", 69.00, "30 g gel", 30, "https://www.carbonendurance.co/precision-fuel-30-gel"
)

# 32Gi – Sports Gel single updated (if you want to use 32Gi store pricing)
gi_updates <- tribble(
    ~Product, ~Brand, ~Price_ZAR, ~Serving_Size, ~Carbs_per_Serving, ~Source_Link,
    "32Gi Sports Gel (27 g single)", "32Gi", 31.99, "27 g gel", 21, "https://32gi.co.za/"
)

# all_products = your big master tibble
# updates_new, precision_updates, gi_updates = your “latest” tibbles
conflicts_prefer(dplyr::last)

latest_updates <- bind_rows(
    all_products, 
    updates_new,
    precision_updates,
    gi_updates
) %>%
    # if the update tables themselves have duplicates, keep the last one
    arrange(Brand, Product) %>%
    group_by(Brand, Product) %>%
    summarise(
        Price_ZAR = last(Price_ZAR),
        Serving_Size = last(Serving_Size),
        Carbs_per_Serving = last(Carbs_per_Serving),
        Source_Link = last(Source_Link),
        .groups = "drop"
    )
# --- VERIFIED PRICE UPDATES / ADDITIONS (checked 2026-03-02) ---


all_products_updated <- all_products %>%
    # drop any old versions of rows that appear in the update set
    anti_join(latest_updates, by = c("Brand", "Product")) %>%
    # add in the updated/new rows
    bind_rows(latest_updates) %>%
